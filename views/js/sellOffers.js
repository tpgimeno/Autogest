...
{            
        ...
        if(document.getElementById('searchCustomerField').value === "");
            ...
            function reqListener()
            {
                ...
                {                          
                   ...
                   {
                       table.innerHTML += "<tr><td>"+response[i]['name']+"</td><td>"+response[i]['city']+"</td><td>"+response[i]['fiscal_id']+"</td><td>"+response[i]['address']+"</td><td><a href=/sells/offers/customer/select?customer_id="+response[i]['id']+"&offer_id={{offer.id}}&vehicle_id={{vehicle.id}}>Select</a></td></tr>";
                   }                               
                }                        
            }
    ...
    {                      
        var request = new XMLHttpRequest();
        request.open('POST', '/sells/offers/vehicle/search', true);
        request.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        ...
        else
        {
            ...
            function reqListener()
            {
                ...
                {                          
                   for(let i = 0; i < response.length ; i++)
                   {
                       table.innerHTML += "<tr><td>"+response[i]['plate']+"</td><td>"+response[i]['vin']+"</td><td>"+response[i]['brand']+"</td><td>"+response[i]['model']+"</td><td><a href=/sells/offers/customer/select?customer_id="+response[i]['id']+"&offer_id={{offer.id}}&customer_id={{customer.id}}>Select</a></td></tr>";
                   }                               
                }                        
            }
            request.addEventListener("load", reqListener);
    ...
    document.getElementById('inputVehiclePrice').value = price_vehicle.format('(0,0.0$)');    
    document.getElementById('inputVehicleDiscount').value = vehicle_discount.format('(0,0.0$)');  
    base_vehicle = numeral(price_vehicle.value() - vehicle_discount.value());
    tva_vehicle = numeral(base_vehicle.value() * tax);   
    document.getElementById('inputVehicleTva').value = tva_vehicle.format('(0,0.0$)');    
    total_vehicle = numeral(base_vehicle.value() + tva_vehicle.value());
    document.getElementById('inputVehicleTotal').value = total_vehicle.format('(0,0.0$)');     
    ...
    document.getElementById('inputSupplyTotal').value = base_supply.format('(0,0.0$)');
    ...
    document.getElementById('inputWorksBase').value = price_works.format('(0,0.0$)');
    tva_works = numeral(price_works.value() * tax);
    document.getElementById('inputWorksTva').value = tva_works.format('(0,0.0$)');
    total_works = numeral(price_works.value() + tva_works.value());
    document.getElementById('inputWorksTotal').value = total_works.format('(0,0.0$)');
    
      
    document.getElementById('inputWorkPrice').value = price_work.format('(0,0.0$)');        
    ...
    document.getElementById('inputWorkTotal').value = total_work.format('(0,0.0$)');    
       

    base = numeral(price_vehicle.value() + price_supplies.value() + price_works.value() + price_components.value() - discount.value());    
    document.getElementById("inputPrice").value = base.format('(0,0.0$)');
    document.getElementById("inputDiscount").value = discount.format('(0,0.0$)');
    tva = numeral(base.value() * tax);
    document.getElementById("inputTva").value = tva.format('(0,0.0$)');
    ...
    document.getElementById("inputPrice").addEventListener("change", function()
    {
        ...
        total = numeral(base.value() + tva.value());  
        document.getElementById("inputTotal").value = total.format('(0,0.0$)');
        document.getElementById("inputPrice").value = price.format('(0,0.0$)');
    });
    ...
    if(json_supplies_response)
    {
        ...
        {            
            ...
            table_supplies.innerHTML += "<tr><td>"+offer_supplies[i]['supplyId']+"</td><td>"+offer_supplies[i]['reference']+"</td><td>"+offer_supplies[i]['name']+"</td><td>"+numeral(offer_supplies[i]['price']).format('(0,0.0$)')+"</td><td>"+offer_supplies[i]['cantity']+"</td><td>"+numeral(offer_supplies[i]['total']).format('(0,0.0$)')+"</td><td><a href=/Intranet/sells/offers/supplies/edit?supplyId="+numeral(offer_supplies[i]['supplyId']).value()+"&offer_id="+numeral(offer.value).value()+"&vehicle_id="+numeral(id_vehicle.value).value()+"&componentId="+numeral(id_component.value).value()+"&work_id="+numeral(id_work.value).value()+"&customer_id="+numeral(id_customer.value).value()+">Edit</a></td><td><a href=/Intranet/sells/offers/supplies/del?supplyId="+numeral(offer_supplies[i]['supplyId']).value()+"&offer_id="+numeral(offer.value).value()+"&vehicle_id="+numeral(id_vehicle.value).value()+"&componentId="+numeral(id_component.value).value()+"&work_id="+numeral(id_work.value).value()+"&customer_id="+numeral(id_customer.value).value()+">Eliminar</a></td>";
    ...
    if(json_components_response)
    {
        ...
        {            
            ...
            table_components.innerHTML += "<tr><td>"+offer_components[i]['componentId']+"</td><td>"+offer_components[i]['reference']+"</td><td>"+offer_components[i]['name']+"</td><td>"+numeral(offer_components[i]['price']).format('(0,0.0$)')+"</td><td>"+offer_components[i]['cantity']+"</td><td>"+numeral(offer_components[i]['total']).format('(0,0.0$)')+"</td><td><a href=/Intranet/sells/offers/components/edit?componentId="+numeral(offer_components[i]['componentId']).value()+"&offer_id="+numeral(offer.value).value()+"&vehicle_id="+numeral(id_vehicle.value).value()+"&customer_id="+numeral(id_customer.value).value()+"&supplyId="+numeral(id_supply.value).value()+"&work_id="+numeral(id_work.value).value()+">Edit</a></td><td><a href=/Intranet/sells/offers/components/del?componentId="+numeral(offer_components[i]['componentId']).value()+"&offer_id="+numeral(offer.value).value()+"&vehicle_id="+numeral(id_vehicle.value).value()+"&customer_id="+numeral(id_customer.value).value()+"&supplyId="+numeral(id_supply.value).value()+"&work_id="+numeral(id_work.value).value()+">Eliminar</a></td>";
    ...
    if(json_works_response.length > 0)
    {
        ...
        {            
            ...
            table_works.innerHTML += "<tr><td>"+offer_works[i]['workId']+"</td><td>"+offer_works[i]['description']+"</td><td>"+numeral(offer_works[i]['price']).format('(0,0.0$)')+"</td><td>"+offer_works[i]['cantity']+"</td><td>"+numeral(offer_works[i]['total']).format('(0,0.0$)')+"</td><td><a href=/Intranet/sells/offers/works/edit?work_id="+numeral(offer_works[i]['workId']).value()+"&offer_id="+numeral(offer.value).value()+"&vehicle_id="+numeral(id_vehicle.value).value()+"&customer_id="+numeral(id_customer.value).value()+"&supplyId="+numeral(id_supply.value).value()+"&componentId="+numeral(id_component.value).value()+">Edit</a></td><td><a href=/Intranet/sells/offers/works/del?work_id="+numeral(offer_works[i]['work_id']).value()+"&offer_id="+numeral(offer.value).value()+"&vehicle_id="+numeral(id_vehicle.value).value()+"&customer_id="+numeral(id_customer.value).value()+"&supplyId="+numeral(id_supply.value).value()+"&componentId="+numeral(id_component.value).value()+">Eliminar</a></td>";
...
{    
    ...
    else
    {
        for(let i = 0; i < offer_supplies.length ; i++)
        {            
            table_supplies.innerHTML += "<tr><td>"+offer_supplies[i]['supplyId']+"</td><td>"+offer_supplies[i]['reference']+"</td><td>"+offer_supplies[i]['name']+"</td><td>"+numeral(offer_supplies[i]['price']).format('(0,0.0$)')+"</td><td>"+offer_supplies[i]['cantity']+"</td><td>"+numeral(offer_supplies[i]['total']).format('(0,0.0$)')+"</td><td><a href=/Intranet/sells/offers/supplies/edit?supplyId="+offer_supplies[i]['supplyId']+"&offer_id="+numeral(offer.value).value()+"&vehicle_id="+numeral(id_vehicle.value).value()+"&componentId="+numeral(id_component.value).value()+"&work_id="+numeral(id_work.value).value()+"&customer_id="+numeral(id_customer.value).value()+">Edit</a></td><td><a href=/Intranet/sells/offers/supplies/del?supplyId="+offer_supplies[i]['supplyId']+"&offer_id="+numeral(offer.value).value()+"&vehicle_id="+numeral(id_vehicle.value).value()+"&componentId="+numeral(id_component.value).value()+"&work_id="+numeral(id_work.value).value()+"&customer_id="+numeral(id_customer.value).value()+">Eliminar</a></td>";
...
function addComponent()
{
    ...
    document.getElementById('inputComponentsBase').value = numeral(price_components).format('(0,0.0$)');
    ...
    document.getElementById("inputTotal").value = total.format('(0,0.0$)');
    var request = new XMLHttpRequest();    
    request.open('POST', '/Intranet/sells/offers/components/add', true);
    request.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");    
...
function addWork()
{
    ...
    else
    {
        for(let i = 0; i < offer_works.length ; i++)
        {            
            table_works.innerHTML += "<tr><td>"+offer_works[i]['workId']+"</td><td>"+offer_works[i]['description']+"</td><td>"+numeral(offer_works[i]['price']).format('(0,0.0$)')+"</td><td>"+offer_works[i]['cantity']+"</td><td>"+numeral(offer_works[i]['total']).format('(0,0.0$)')+"</td><td><a href=/Intranet/sells/offers/works/edit?work_id="+offer_works[i]['workId']+"&offer_id="+numeral(offer.value).value()+"&vehicle_id="+numeral(id_vehicle.value).value()+"&customer_id="+numeral(id_customer.value).value()+"&supplyId="+numeral(id_supply.value).value()+"&componentId="+numeral(id_component.value).value()+">Edit</a></td><td><a href=/Intranet/sells/offers/works/del?work_id="+numeral(offer_works[i]['work_id']).value()+"&offer_id="+numeral(offer.value).value()+"&vehicle_id="+numeral(id_vehicle.value).value()+"&customer_id="+numeral(id_customer.value).value()+"&supplyId="+numeral(id_supply.value).value()+"&componentId="+numeral(id_component.value).value()+">Eliminar</a></td>";
            suma_prices_works += numeral(numeral(price_works).value() + offer_works[i]['total']).value();           
        }  
    }       
    ...
    document.getElementById('inputWorksBase').value = numeral(price_works).format('(0,0.0$)');
    document.getElementById('inputWorksTva').value = numeral(tva_works).format('(0,0.0$)');
    document.getElementById('inputWorksTotal').value = numeral(total_works).format('(0,0.0$)');
    discount = numeral(document.getElementById('inputDiscount').value);
    ...
    document.getElementById("inputTotal").value = total.format('(0,0.0$)');
    var request = new XMLHttpRequest();    
    request.open('POST', '/sells/offers/works/add', true);
    request.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");    

 ...
 */
class SuppliesReport extends FPDF
{    
    public function Header()
    {
        ...
        $this->SetY(42);
        $this->Cell(0, 8, 'RECAMBIOS', 0, 1, 'L', 1);
        $this->SetY(50);
...
//        var_dump($data);die();
        $this->SetFillColor(255);
        $this->SetTextColor(0);
        for($i=0;$i<count($data['supplies']);$i++){                          
            $this->SetFont('Arial','', 8);
            $this->Cell(20, 6, $data['supplies'][$i]->id, 0,0,'C', 1);
            $this->Cell(35, 6, $data['supplies'][$i]->ref, 0,0,'L', 1); 
            $this->Cell(85, 6, $data['supplies'][$i]->name, 0,0,'L', 1);                              
            $this->Cell(25, 6, number_format($this->tofloat($data['supplies'][$i]->pvc),2,'.',',')." ".EURO, 0,0,'C', 1);
            $this->Cell(25, 6, number_format($this->tofloat($data['supplies'][$i]->pvp),2,'.',',')." ".EURO, 0,1,'C', 1);
